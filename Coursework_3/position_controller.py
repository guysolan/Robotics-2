...
def main(task):
    ...
    left_rpy = [0, np.pi, 0]    # Replace the roll, pitch, and yaw value with the correct value (in radians)
    left_arm.servo_to_pose(left_xyz, left_rpy)
    
    # Go to the starting Pose for right arm
    right_xyz = [0.5, -0.5, 0.25]
    right_rpy = [0, np.pi, 0]   # Replace the roll, pitch, and yaw value with the correct value (in radians)
    ...
    if task == 'initPose':
        ...
        left_arm.servo_to_pose([0.75, 0.5, 0.0], left_rpy)
        left_arm.servo_to_pose([0.75, -0.5, 0.0], left_rpy)
        left_arm.servo_to_pose([0.75, 0, -0.05], left_rpy)
        left_arm.gripper_open()
        
        left_arm.servo_to_pose([0.75, 0, 0.25], left_rpy)
        left_arm.servo_to_pose([0.75, 0.5, 0.25], left_rpy)
        
        right_rpy = [np.pi, 0, np.pi]
        right_arm.servo_to_pose([0.75, 0, 0.25], right_rpy)
        right_arm.servo_to_pose([0.75, 0, -0.05], right_rpy)
        right_arm.gripper_close()
        
        right_arm.servo_to_pose([0.75, 0, 0.0], right_rpy)
        right_arm.servo_to_pose([0.75, -0.5, 0.0], right_rpy)
        right_arm.servo_to_pose([0.75, -0.5, -0.05], right_rpy)
        right_arm.gripper_open()
        
        right_arm.servo_to_pose([0.75, -0.5, 0.25], right_rpy)
        ...
        left_arm.servo_to_pose([0.75, 0.5, 0.15], left_rpy)
        left_arm.servo_to_pose([0.75, 0.1, 0.15], [float(np.pi/2), np.pi, 0])
        
        right_arm.servo_to_pose([0.75, -0.1, 0.15], [float(-np.pi/2), np.pi, 0])
        
        left_arm.servo_to_pose([0.75, 0, 0.15], [float(np.pi/2), np.pi, 0])
        right_arm.servo_to_pose([0.75, 0, 0.15], [float(-np.pi/2), np.pi, 0])
        
        right_arm.gripper_close()
        left_arm.gripper_open()
        
        left_arm.servo_to_pose([0.75, 0.1, 0.15], [float(np.pi/2), np.pi, 0])
        right_arm.servo_to_pose([0.75, -0.1, 0.1], [float(-np.pi/2), np.pi, 0])
        right_arm.servo_to_pose([0.75, 0, 0.0], right_rpy)
        right_arm.servo_to_pose([0.75, -0.5, 0.0], right_rpy)
        right_arm.servo_to_pose([0.75, -0.5, -0.05], right_rpy)
        right_arm.gripper_open()
