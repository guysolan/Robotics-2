...
class VelocityController(b_pykdl.baxter_kinematics):
    ...
    def ee_IK_solver(self, joint_values, P_des, quat_des, dt, nullspace_en, vel_elbow):
        ...
        dP = (P_des - P)/dt  # linear displacement divide by time resulting in angular velocity
        dw = r*(delta_angle/dt) # angular displacement dived by time resulting in angular velocity       
        ...
        J_ee = self.jacobian(joint_values)    # the jacobian is built using the structure of the robot and the current joint angles
                                              # this jacobian is from the end-effector as it accounts for the whole robot structure and all joint angles
        J_ee = np.asarray(J_ee) # convert to np.array

        J_pinv = DPinv(J_ee, eps=1e-10) # compute the pseudoinverse of J_ee
        qd = np.matmul(J_pinv, twist) # matrix multiplication between pseudoinverse of the end-effector jacobian and the twist to compute joint velocitie
        ...
        if nullspace_en:
            ...
            I = np.eye(nj)
            Proj = I - np.matmul(J_pinv, J_ee)   # this is the null space projector, N. Replace [] with your calculation.
...
def main(task):
    ...
    xyz_des_pick = [0.75, 0, 0.93]       # Point to close grippers to grab the object
    xyz_des_circle = [0.75, 0.1, 1.23]     # The point at which to start drawing the circle
    rpy_des = [0, float(np.pi/2), 0]  # Define the end effector angle relative to the base axes in order to grab the objetc from behind
    if task == "go2pose":
