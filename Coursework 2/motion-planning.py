...
class MotionPlanner():
    ...
    def setup_waypoints(self):
        ...
        waypoints0 = np.array([[0, -6],[2.138, -2.210],[3.129, 4.955],[3.129, 7.396],[2.47,8.0]])
                              
        waypoints1 = np.array([[0, -6],[2.077, -3.664],[3.129, 4.955],[3.129, 7.396],[2.47,8.0]])
        
        waypoints2 = np.array([[0, -6],[2.077, -3.664],[6.08, -0.9704],[7.911, 4.951],[2.47,8.0]])
                                          
        waypoints3 = np.array([[0, -6],[2.077, -3.664],[3.942, 3.281],[7.911, 4.851],[2.47,8.0]])
        
        waypoints4 = np.array([[0, -6],[2.138, -2.210],[3.942, 3.281],[7.911, 4.851],[2.47,8.0]])
                              
        waypoints = np.array([waypoints0,waypoints1,waypoints2,waypoints3,waypoints4])
        distances = np.zeros(len(waypoints))
       
        for index, ele in enumerate(waypoints):
            for i in range(len(ele)-1):
                dif = np.subtract(ele[i], ele[i+1])
                sqr = np.power(dif, 2)
                add = np.sum(sqr)
                dis = np.sqrt(add)
                distances[index] += dis 
        ...
        waypoints = waypoints[np.argmin(distances)][1:-1,:]     
...
#        plt.show()
    ...
    def generate_random_points(self, N_points):
        ...
        while N_accepted < N_points:    # keep generating points until N_points have been accepted
            ...
            for i,ele in enumerate(pixel_points):
                rejected[i] = self.pixel_map[int(ele[1]),int(ele[0])]
    ...
    def check_collisions(self, pointA, pointB):
        ...
        #print("PA: " + str(pointA) + "  PB: " + str(pointB))
        dif = np.subtract(pointB, pointA)
        sqr = np.power(dif, 2)
        add = np.sum(sqr)
        distance = np.sqrt(add)
        
        # Calculate the UNIT direction vector pointing from pointA to pointB
        #print(np.divide(dif,distance))
        direction = np.divide(dif,distance)
        # Choose a resolution for collision checking
        resolution = 1.0   # resolution to check collision to in m
    ...
    def dijkstra(self, graph, edges):
        ...
        # Extract the nodes into a list from the graph dictionary
        nodes = list(graph.keys())
        
        # Create a dataframe of unvisited nodes
        
        # Initialise each cost to a value larger than the greatest feasible value
        initial_cost = 99.0
        ...
        unvisited.loc[[str(initial_position)], ['Cost']] = 0.0 # .loc accesses the data frame by boolean or, as here, column name
        ...
        while str(goal_node) not in visited.index.values:
            ...
            #Separate the information from the current node into a variable for each column of the dataframe
            ...
            for next_node_name, edge_cost in zip(connected_nodes, connected_edges): # zip method allows the two lists of ajoined distances and nodes to be looped over simultaneously.
                
                ## Plot the map for the next connected nodes plot
                # plt.imshow(self.pixel_map, vmin=0, vmax=1, origin='lower')
                ...
                if next_node_name not in visited.index.values:  # if we haven't visited this node before
                    ...
                    ## Make next node into a compatible numpy array with function defined at the bottom of the script
                    # next_node_name_np = make_np_array(next_node_name)
                    
                    ## plot the position of the next node as a white spot
                    # node_pixel = self.map_position(next_node_name_np)
                    # plt.scatter(node_pixel[0, 0], node_pixel[0, 1], c='w')
                        
                    ## Make current node into a compatible numpy array
                    # current_node_name_np = make_np_array(current_node_name)
                    
                    ## create an array from current node to next node
                    # from_current = np.array([current_node_name_np,next_node_name_np])
                    
                    ## plot the path from the current node to the next node
                    # available_pixels = self.map_position(from_current)
                    # plt.plot(available_pixels[:, 0], available_pixels[:, 1], c='b')
                    ...
                    if next_cost_trial < next_cost:
                        ...
                        ## Change plot color to Cyan if it is a new lowest cost
                        #plt.plot(available_pixels[:, 0], available_pixels[:, 1], c='c')
                     
            ## Show the plot of connected nodes for each current_node
            # plt.show()
...
def make_np_array(node_name):
    # Make current_node_name into a list (from a string)
    res = node_name.strip('][').split(' ')

    # Remove any empty strings generated in the list from spaces in current_node_name
    ...
    # Create a numpy array from the current node to the next node.
    cnn_np = np.array([current_node_x, current_node_y])
